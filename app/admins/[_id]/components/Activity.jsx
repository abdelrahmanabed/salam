...
const Activities = () => {
 const {theAdmin} = useAdmin()
  ...
  const allNotes = theAdmin?.notes|| [];

  // Get pinned and regular notes directly from theAdmin
  ...
  const allActivities = theAdmin ? [
    ...(theAdmin.abnormalEvents || []).map(event => ({
      ...event,
