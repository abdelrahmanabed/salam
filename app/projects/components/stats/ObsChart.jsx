...
const ObsChart = () => {
  ...
  const prepareChartData = () => {
    ...
    const observationsData = project.calendar.flatMap(year =>
      year.months.flatMap(month =>
        month.weeks.flatMap(week =>
          week.days.flatMap(day => day.observations)
        )
      )
    );

    const filteredData = calculateFilteredData(observationsData, filter);
  ...
  return (
    <div className="h-52 w-full p-2 bg-boxcolor dark:bg-blackgrey gap-1 flex flex-col rounded-main ">
      <div className="flex items-center justify-between text-subtextcolor ">
          <span className="p-1 bg-orangecolor text-xs rounded-full">Observations</span>
          ...
          {['WTD', 'MTD', 'YTD', 'PTD'].map(type => (
            <button
              ...
              className={`px-2 py-1 text-xs duration-300 rounded-full ${
                filter === type ? 'bg-orangecolor text-subtextcolor' : 'bg-hovercolor text-textcolor'
              }`}
            >
              {type}
            </button>
